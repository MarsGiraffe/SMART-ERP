...
$(document).ready(function() {
	...
	var store_num = "${employee.store_num}";
        $.ajax({
            url: "${path}/ssPrdList?store_num=" + store_num,
            type: "GET",
            ...
            success: function(data) {
                ...
                /* 권한에 따른 재고 수정 여부 */
                var empValid = "${employee.job_level}" === "점주" || "${employee.job_level}" === "매니저" || "${employee.job_level}" === "직원";
                var HQValid = "${employee.job_level}" === "사장" || "${employee.job_level}" === "팀장" || "${employee.job_level}" === "본사직원";
                if (HQValid) {
...
/* 	            $("#selectAllCheckbox").on("change", function() {
            ...
            /* 재고수정 버튼 클릭시 재고 업데이트 */
             $(document).on("click", "#uptStock", function() {
               ...
               var newStockValue = parseInt($(this).closest("tr").find("input[name='storage_count']").val(), 10);
...
<div class="container-fluid">
   ...
   <div class="d-sm-flex align-items-center justify-content-between mb-4">
      <h1 class="h3 mb-0 text-gray-800">${employee.store_name}</h1>
   </div>
